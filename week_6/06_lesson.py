...
Формат ввода
 В первой строке вводится число n - количество селений (1 <= n <= 100000). Вторая строка содержит n
...
найденной для предыдущего города. Аналогично, не нужно искать подходящее бомбоубежище до конца списка бомбоубежищ:
...
def find_value(x):
    if x < shelters_tup[0][1]:
        return shelters_tup[0][0]
    if x > shelters_tup[-1][1]:
    ...
    while r - l > 1:
        m = (r + l) >> 1
        if shelters_tup[m][1] < x:
            l = m
        else:
            r = m

    if x - shelters_tup[l][1] < shelters_tup[r][1] - x:
        return shelters_tup[l][0]
    else:
        return shelters_tup[r][0]


resList = [find_value(v) for v in villages]
print(*resList)
